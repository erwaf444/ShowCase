...
public class TimeSystem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (SteamManager.Initialized)
        {
            if (day == 10)
            {
                gameManagerScript.UnlockAchievement("GO_10_DAYS");
            }

            if (day == 100)
            {
                gameManagerScript.UnlockAchievement("GO_100_DAYS");
            }

            if (day == 1000)
            {
                gameManagerScript.UnlockAchievement("GO_1000_DAYS");
            }

            if (year == 2026)
            {
                gameManagerScript.UnlockAchievement("NEW_YEAR");
            }
        }
    ...
    public void NextDay()
    {
        if (isDayEnded)
        {
            ...
            gameManagerScript.SaveMoney();
            playerScript.SaveExp();
    ...
    IEnumerator EndDayRoutine()
    {
        ...
        incomeDialogueText.text = incomeDialogue.incomeDialogue[Random.Range(0, incomeDialogue.incomeDialogue.Length)];
        outlayDialogueText.text = outlayDialogue.outlayDialogue[Random.Range(0, outlayDialogue.outlayDialogue.Length)];
    ...
    void CheckRentPayment()
    {
        ...
        if (day == 5)
        {
            ...
            gameManagerScript.UnlockAchievement("FIRST_RENT");
    ...
    void TriggerRandomOtherIncomeEvent()
    {
        ...
        int randomIncome = Random.Range(0, 500);
    ...
    {
        // 生成一个随机收入数值，例如在 10 到 100 之间
        int randomOutlay = Random.Range(0, 200);
    ...
    public void SaveDays()
    {
        if (SteamManager.Initialized)
        {
            SteamUserStats.SetStat("Days", day);
            bool success = SteamUserStats.StoreStats();
            
            if (success)
            {
                Debug.Log("Days saved to Steam Cloud: " + day);
            }
            else
            {
        ...
        {
            Debug.LogWarning("Steam is not initialized. Unable to save Days.");
        }
    }
    ...
    public void LoadDays()
    {
        ...
        if (SteamManager.Initialized)
        {
            ...
            {
                day = storedDays;
                Debug.Log("day loaded from Steam Cloud: " + day);
            }
            else
            {
    ...
    public void SaveMonth()
    {
        if (SteamManager.Initialized)
        {
            SteamUserStats.SetStat("Month", month);
            bool success = SteamUserStats.StoreStats();
            
            if (success)
            {
                Debug.Log("Month saved to Steam Cloud: " + month);
            }
            else
            {
                Debug.LogWarning("Failed to store stats to Steam Cloud.");
        ...
        {
            Debug.LogWarning("Steam is not initialized. Unable to save Month.");
        }
    }
    ...
    public void LoadMonth()
    {
        ...
        if (SteamManager.Initialized)
        {
            // 從Steam Cloud中獲取金錢數據
            int storedMonth;
            bool hasStat = SteamUserStats.GetStat("Month", out storedMonth);
        
            if (hasStat)
            {
                month = storedMonth;
                Debug.Log("month loaded from Steam Cloud: " + month);
            }
            else
            {
                Debug.LogWarning("Stat 'month' not found or could not be retrieved.");
            }
        }
        else
        {
            Debug.LogWarning("Steam is not initialized. Unable to load month.");
        }
    }
    ...
    public void SaveYear()
    {
        if (SteamManager.Initialized)
        {
            SteamUserStats.SetStat("Year", year);
            bool success = SteamUserStats.StoreStats();
            
            if (success)
            {
                Debug.Log("Year saved to Steam Cloud: " + year);
            }
            else
            {
                Debug.LogWarning("Failed to store stats to Steam Cloud.");
        ...
        {
            Debug.LogWarning("Steam is not initialized. Unable to save Year.");
        }
    }
    ...
    public void LoadYear()
    {
        ...
        if (SteamManager.Initialized)
        {
            // 從Steam Cloud中獲取金錢數據
            int storedYear;
            bool hasStat = SteamUserStats.GetStat("Year", out storedYear);
        
            if (hasStat)
            {
                year = storedYear;
                Debug.Log("year loaded from Steam Cloud: " + year);
            }
            else
            {
                Debug.LogWarning("Stat 'year' not found or could not be retrieved.");
            }
        }
        else
        {
            Debug.LogWarning("Steam is not initialized. Unable to load year.");
        }
    }

    public void SaveCleanValue()
    {
        if (SteamManager.Initialized)
        {
            SteamUserStats.SetStat("Clean_Value", bookFirstPageScript.cleanValue);
            bool success = SteamUserStats.StoreStats();
            
            if (success)
            {
                Debug.Log("Clean_Value saved to Steam Cloud: " + bookFirstPageScript.cleanValue);
            }
            else
            {
                Debug.LogWarning("Failed to store stats to Steam Cloud.");
        ...
        {
            Debug.LogWarning("Steam is not initialized. Unable to save Clean_Value.");
        }
    }

    public void LoadCleanValue()
    {

        if (SteamManager.Initialized)
        {
            // 從Steam Cloud中獲取金錢數據
            int storedCleanValue;
            bool hasStat = SteamUserStats.GetStat("Clean_Value", out storedCleanValue);
        
            if (hasStat)
            {
                bookFirstPageScript.cleanValue = storedCleanValue;
                Debug.Log("cleanValue loaded from Steam Cloud: " + bookFirstPageScript.cleanValue);
            }
            else
            {
                Debug.LogWarning("Stat 'cleanValue' not found or could not be retrieved.");
            }
        }
        else
        {
            Debug.LogWarning("Steam is not initialized. Unable to load cleanValue.");
        }
    }

    public void SaveFoodFreshValue()
    {
        if (SteamManager.Initialized)
        {
            SteamUserStats.SetStat("Food_Fresh_Value", bookFirstPageScript.foodFreshValue);
            bool success = SteamUserStats.StoreStats();
            
            if (success)
            {
                Debug.Log("Food_Fresh_Value saved to Steam Cloud: " + bookFirstPageScript.foodFreshValue);
            }
            else
            {
                Debug.LogWarning("Failed to store stats to Steam Cloud.");
        ...
        {
            Debug.LogWarning("Steam is not initialized. Unable to save Food_Fresh_Value.");
        }
    }

    public void LoadFoodFreshValue()
    {

        if (SteamManager.Initialized)
        {
            // 從Steam Cloud中獲取金錢數據
            int storedFoodFreshValue;
            bool hasStat = SteamUserStats.GetStat("Food_Fresh_Value", out storedFoodFreshValue);
        
            if (hasStat)
            {
                bookFirstPageScript.foodFreshValue = storedFoodFreshValue;
                Debug.Log("FoodFreshValue loaded from Steam Cloud: " + bookFirstPageScript.foodFreshValue);
            }
            else
            {
                Debug.LogWarning("Stat 'FoodFreshValue' not found or could not be retrieved.");
            }
        }
        else
        {
            Debug.LogWarning("Steam is not initialized. Unable to load FoodFreshValue.");
        }
    }

    public void SaveCustomerHappyValue()
    {
        if (SteamManager.Initialized)
        {
            SteamUserStats.SetStat("Customer_Happy_Value", bookFirstPageScript.customerHappyValue);
            bool success = SteamUserStats.StoreStats();
            
            if (success)
            {
                Debug.Log("Customer_Happy_Value saved to Steam Cloud: " + bookFirstPageScript.customerHappyValue);
            }
            else
            {
                Debug.LogWarning("Failed to store stats to Steam Cloud.");
        ...
        {
            Debug.LogWarning("Steam is not initialized. Unable to save Customer_Happy_Value.");
        }
    }

    public void LoadCustomerHappyValue()
    {

        if (SteamManager.Initialized)
        {
            // 從Steam Cloud中獲取金錢數據
            int storedCustomerHappyValue;
            bool hasStat = SteamUserStats.GetStat("Customer_Happy_Value", out storedCustomerHappyValue);
        
            if (hasStat)
            {
                bookFirstPageScript.customerHappyValue = storedCustomerHappyValue;
                Debug.Log("customerHappyValue loaded from Steam Cloud: " + bookFirstPageScript.customerHappyValue);
            }
            else
            {
                Debug.LogWarning("Stat 'customerHappyValue' not found or could not be retrieved.");
            }
        }
        else
        {
            Debug.LogWarning("Steam is not initialized. Unable to load customerHappyValue.");
        }
    }

    public void SaveMakedBreads()
    {
        if (SteamManager.Initialized)
        {
            SteamUserStats.SetStat("Bread", gameManagerScript.makedBreads);
            bool success = SteamUserStats.StoreStats();
            
            if (success)
            {
                Debug.Log("makedBreads saved to Steam Cloud: " + gameManagerScript.makedBreads);
            }
            else
            {
                Debug.LogWarning("Failed to store stats to Steam Cloud.");
        ...
        {
            Debug.LogWarning("Steam is not initialized. Unable to save makedBreads.");
        }
    }

    public void LoadMakedBreads()
    {
        ...
        if (SteamManager.Initialized)
        {
            // 從Steam Cloud中獲取金錢數據
            int storedBreads;
            bool hasStat = SteamUserStats.GetStat("Bread", out storedBreads);
        
            if (hasStat)
            {
                gameManagerScript.makedBreads = storedBreads;
                Debug.Log("makedBreads loaded from Steam Cloud: " + gameManagerScript.makedBreads);
            }
            else
            {
                Debug.LogWarning("Stat 'makedBreads' not found or could not be retrieved.");
            }
        }
        else
        {
            Debug.LogWarning("Steam is not initialized. Unable to load makedBreads.");
        }
    }

    public void SaveAccumulateDay()
    {
        if (SteamManager.Initialized)
        {
            SteamUserStats.SetStat("Accumulate_Day", AccumulateDay);
            bool success = SteamUserStats.StoreStats();
            
            if (success)
            {
                Debug.Log("Accumulate_Day saved to Steam Cloud: " + AccumulateDay);
            }
            else
            {
                Debug.LogWarning("Failed to store stats to Steam Cloud.");
        ...
        {
            Debug.LogWarning("Steam is not initialized. Unable to save Accumulate_Day.");
        }
    }

    public void LoadAccumulateDay()
    {

        if (SteamManager.Initialized)
        {
            // 從Steam Cloud中獲取金錢數據
            int storedAccumulateDay;
            bool hasStat = SteamUserStats.GetStat("Accumulate_Day", out storedAccumulateDay);
        
            if (hasStat)
            {
                AccumulateDay = storedAccumulateDay;
                Debug.Log("AccumulateDay loaded from Steam Cloud: " + gameManagerScript.makedBreads);
            }
            else
            {
                Debug.LogWarning("Stat 'AccumulateDay' not found or could not be retrieved.");
            }
        }
        else
        {
            Debug.LogWarning("Steam is not initialized. Unable to load AccumulateDay.");
        }
    }
