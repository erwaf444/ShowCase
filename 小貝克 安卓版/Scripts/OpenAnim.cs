...
public class OpenAnim : MonoBehaviour
{
    ...
    async void Start()
    {
        ...
        StartCoroutine(WaitTimeSystemReady());
        
        StartCoroutine(FlickerArrows());
    }

    IEnumerator WaitTimeSystemReady()
    {
        yield return new WaitUntil(() => TimeSystem.isReady);
        Debug.Log("TimeSystem.day: " + TimeSystem.day + ", TimeSystem.month: " + TimeSystem.month + ", TimeSystem.year: " + TimeSystem.year);
    ...
    void Update()
    {
        if (canStartOpenAnim)
        {
            ...
            if (Input.GetKeyDown(KeyCode.Space)) // 電腦平台使用空格鍵
            {
                AdvanceDialogue();
            }
        #elif UNITY_ANDROID || UNITY_IOS
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began) // 手機平台使用觸控
            {
                AdvanceDialogue();
            }
        #endif
    ...
    #region unityCloud
    public async void SaveOpenAnimState()
    {
        int valueToSave = OpenAnimIsEnd ? 1 : 0;
        await SaveData("OpenAnimIsEnd", valueToSave);
    }
    
    public async void LoadOpenAnimState()
    {
        int loadedValue = await LoadData("OpenAnimIsEnd", 0);
        OpenAnimIsEnd = loadedValue == 1;
    }
    ...
    try
        {
            ...
            {
                var value = savedData[key]; // 返回的是字符串
